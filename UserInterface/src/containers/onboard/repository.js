...
class Repository extends React.Component{
    ...
    handleShowModal = (modalId) => {
        if(modalId === 'addrepositoryModal')this.setState({showAddModal:!this.state.showAddModal})
        if(modalId === 'editrepositoryModal')this.setState({showEditModal:!this.state.showEditModal})
    ...
    handleDelete = (event) => {
        ...
        fetch(`###REACT_APP_PLATFORM_URL###/onboard/repo`, requestOptions)
            .then((response) => {
                ...
                console.log(response.status);
                (response.status == 200) ? this.setState({repo:updatedArray}) : this.setState({status:'There was an unknown error', msgClass:'errorMessage'});     
    ...
    handleEditRepository = (name,vendor,index) => {
        this.setState({
            editContent1:name,
            editContent2:vendor,
            edit:true,
    ...
    handleValidation1(){
        ...
        // invalid Repo username
        if(typeof fields.repo_username !== "undefined"){
            if(!fields.repo_username.match(/^[A-Za-z0-9_-]/)){
                formIsValid = false;
                errors.repo_username = "invalid name";
            }        
        }
        // Password              
        if(!fields.repo_password){
        ...
        // invalid password
        // if(typeof fields.repo_password !== "undefined"){
        //     if(!fields.repo_password.match(/(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.*[!@#\$%\^&\*])(?=.{6,})/)){
        //         formIsValid = false;
        //         errors.repo_password = "invalid password";
        //     }        
        // }
        this.setState({errors});
        return formIsValid;
    ...
    render(){
                ...
                return <tr className="" key={index} >
                    ...
                    <td class="text-center">
                        <div class="dev-actions">
                            <a href="javascript:void(0)" data-toggle="modal" data-target="#myUpdateRepositoryModal" onClick={() => this.handleEditRepository(value.repo_name,value.rep_vendor,index)}><img src={require("images/edit.svg")} alt="Edit"/></a>
        ...
        const addRepositoryModal = <form className="modalbody" id='addRepositoryForm' onSubmit= {this.contactSubmit.bind(this)}> 
            <div>  
                ...
                <input type="text" name="repo_username" className="form-control" placeholder="Enter Username" onChange={this.handleChange.bind(this, "repo_username")}/>
        ...
        const updateRepositoryModal = <form className="modalbody" id='updateRepositoryForm'> 
            <div> 
                <label className="form-label">Name</label>
                <input type="text"  name="repo_name" value={this.state.editContent1} className="form-control" placeholder="Enter Name"/> 

                <br/>
                <label className="form-label">Vendor</label>
                <select className="form-control" name="repo_vendor" value={this.state.editContent2}>
                ...
                <br/>
                <label className="form-label">URL</label>
                <input type="url" name="repo_url" className="form-control" placeholder="Enter URL"/>
                        ...
                        <div className="modal-dialog modal-dialog-centered" role="document">
                            <div className="modal-content">
                                ...
                                <div className="modal-footer">
                                    ...
                                    <button type="button" className="btn btn-primary" onClick={this.handleUpdateRepository}>Submit</button>
                                </div>
                            </div>
                        </div>
                    </div>
